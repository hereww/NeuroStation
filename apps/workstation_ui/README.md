...
- SSVEP 参数页要求选择左眼或右眼，按物理横坐标显示自动的左右屏映射，并预览自动生成的 `原名称_左眼` / `原名称_右眼` 数据集名称；刺激时长、休息、重复数和绝对保存目录仍可编辑。
...
默认四个目标频率为 10 / 12 / 15 / 20 Hz，刺激 5 秒、休息 3 秒、每目标重复 3 次，共 12 个试次。每次只采集一只眼：最左显示器绑定左眼，最右显示器绑定右眼，选中屏幕闪烁，另一块屏幕保持黑屏；正式可视采集至少需要两块显示器。

...
当前为可运行的**模拟版**：不连接设备、不显示真实高频闪烁。独立入口只做 UI mock，不创建采集文件；项目根目录的集成入口 `workstation.py` 会创建协议、事件、会话和清单元数据，但仍不会生成或伪造真实 EEG 样本。界面中的 Cyton、COM5、丢包、磁盘容量为示例状态；样本与 Marker 数由协议计算。
...
也支持 `python apps/workstation_ui/main.py`。默认中文；英文启动参数为 `--language en-US`。缺少 PySide6 时会输出安装指引并以状态码 2 退出，不影响纯逻辑测试。
...
- SSVEP 参数页可编辑参与者、数据集名、刺激时长、休息、重复数、模拟倍率、绝对保存目录；自动计算预计试次与时长。
- 开始任务先经过真实 5 秒倒计时；之后按选定倍率推进静态模拟。运行页展示正式时间轴、试次、目标、频率、阶段、采集与 Marker 状态。
- 完成页展示样本/事件、时长、模拟状态、绝对目标路径；点击“系统工作站 / 数据集”可查看并突出最近完成项。
...
最后一个试次后不追加休息。准备倒计时不计入 EEG 样本。默认 8× 模拟运行，完整演示约需 17 秒，正式预计时间保持 98 秒。60 Hz 是原型参数，正式刺激必须单独验证刷新率与逐帧时序。
...
| `pages.py` | 页面及用户动作信号 |
| `components.py` | 通用按钮、信息区、静态波形、静态刺激预览 |

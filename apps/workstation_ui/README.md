...
- SSVEP 参数页要求选择左眼或右眼，预览主显示器左右半区的分配和自动生成的眼别数据集名称；刺激时长、休息、重复数和绝对保存目录仍可编辑。
...
默认四个目标频率为 10 / 12 / 15 / 20 Hz，刺激 5 秒、休息 3 秒、每目标重复 3 次，共 12 个试次。每次只采集一只眼：主显示器的左半区用于左眼，右半区用于右眼；仅选中半区呈现四目标刺激，另一半保持黑屏，单屏即可采集。

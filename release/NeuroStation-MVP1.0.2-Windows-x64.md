...
- 参考 OpenBCI GUI：按 Cyton 包序号统计真实丢包；包序列连续时，主机时间戳抖动和通道质量只记录为 warning，不单独阻止采集；严重降级或失败仍需复核/阻断；
...
生产采集模式固定为 OpenBCI Cyton 实机。没有连接硬件时只能查看历史记录、导入 OpenBCI 文件或运行诊断，不会生成模拟 EEG。真实采集前，请先复核通道配置、参考/BIAS、电极连接、显示器刷新率和光学时序，并在设备页运行硬件预检。

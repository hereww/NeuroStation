...
- Carries forward the complete dataset preview: every source data row and every raw field are available;
- preserves original EEG, `other_ch`, `analog`, timestamp precision, sample-index order, and auxiliary fields;
- uses a virtualized read-only Qt table model for long recordings;
- updates product, package, Windows file, and SBOM metadata to `1.02` / `1.0.2`.
...
- portable `NeuroStation.dist` directory and `NeuroStation-1.02-Windows-x64.zip` archive.
